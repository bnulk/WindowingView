...
{
    public class Texture_StbImageSharp
    {
        ...
        /// </summary>
        public unsafe Texture_StbImageSharp(GL gl, string path)
        {
            _gl = gl;
            ...
            // 固定数据指针，用于传入 glTexImage2D
            fixed (byte* ptr = result.Data)
            {
                // 创建纹理图像并上传到 GPU
                _gl.TexImage2D(TextureTarget.Texture2D,          // 目标类型：2D 纹理
                               0,                                // mipmap 级别：0 = base level
                               InternalFormat.Rgba,              // GPU 内部格式：RGBA
                               (uint)result.Width,               // 宽度（像素）
                               (uint)result.Height,              // 高度（像素）
                               0,                                // 边框宽度，必须为 0
                               PixelFormat.Rgba,                 // 输入数据格式
                               PixelType.UnsignedByte,           // 每通道一个无符号字节
                               ptr);                             // 指向像素数据的裸指针
            }
        ...
        /// height —— 纹理高度（像素）  
        /// </summary>
        public unsafe Texture_StbImageSharp(GL gl, Span<byte> data, uint width, uint height)
        {
            _gl = gl;
